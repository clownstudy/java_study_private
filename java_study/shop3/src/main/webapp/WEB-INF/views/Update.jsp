	...
	<tbody >
		...
		<tr>  
			...
			<td class="tbodystyle">
				<select name="m_job">
					<option value="0">
					<c:if test='${mdto.m_job==null}'>selected</c:if>선택하세요.</option>
					<option value="회사원">
					<c:if test='${mdto.m_job=="회사원"}'>selected</c:if>회사원</option>
					<option value="연구전문직">
					<c:if test='${mdto.m_job=="연구전문직"}'>selected</c:if>연구전문직</option>
					<option value="교수학생">
					<c:if test='${mdto.m_job=="교수학생"}'>selected</c:if>교수학생</option>
					<option value="일반자영업">
					<c:if test='${mdto.m_job=="일반자영업"}'>selected</c:if>일반자영업</option>
					<option value="공무원">
					<c:if test='${mdto.m_job=="공무원"}'>selected</c:if>공무원</option>
					<option value="의료인">
					<c:if test='${mdto.m_job=="의료인"}'>selected</c:if>의료인</option>
					<option value="법조인">
					<c:if test='${mdto.m_job=="법조인"}'>selected</c:if>법조인</option>
					<option value="종교,언론,예술인">
					<c:if test='${mdto.m_job=="종교,언론,예술인"}'>selected</c:if>종교,언론,예술인</option>
					<option value="농,축,수산,광업인">
					<c:if test='${mdto.m_job=="농,축,수산,광업인"}'>selected</c:if>농,축,수산,광업인</option>
					<option value="주부">
					<c:if test='${mdto.m_job=="주부"}'>selected</c:if>주부</option>
					<option value="무직">
					<c:if test='${mdto.m_job=="무직"}'>selected</c:if>무직</option>
					<option value="기타">
					<c:if test='${mdto.m_job=="기타"}'>selected</c:if>기타3</option>
				</select>
			</td>
		</tr>
		<tr> 
